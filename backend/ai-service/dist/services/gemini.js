...
function readJSON(file) {
    const p = path_1.default.join(__dirname, "..", "..", "knowledge", file);
    return JSON.parse(fs_1.default.readFileSync(p, "utf-8"));
}
const university = (() => {
    try {
        return readJSON("university.json");
    }
    catch {
        return {};
    }
...
const teacherFallback = (() => {
    try {
        return readJSON("teachers.sample.json");
    }
    catch {
        return { teachers: [] };
    }
...
async function loadTeachers() {
    ...
    try {
        ...
        if (!res.ok)
            throw new Error(`Teachers fetch failed: ${res.status}`);
        const data = await res.json();
        const mapped = (data?.teachers || data || []).map((t) => ({
...
async function generateAnswer({ message, userId, role }) {
